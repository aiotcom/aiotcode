...
void SampleApp_Init( uint8 task_id )
{
  ...
  printf("i am coordinator for motor\r\n");//串口打印
  OLED_P8x16Str(0,0,"coor-motor");
...
uint16 SampleApp_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  {//检测到按键按下，发送指令
	...
	if(KeyValue == KEY_S1)
	{//按键S1->开启
		Coor_Send_Motor_ControlCmd(Motor_OFF);
	}
	else if(KeyValue == KEY_S2)
	{//按键S2->关闭
		Coor_Send_Motor_ControlCmd(Motor_ON);
	}
...
{ 
  ...
  case BIND_RSP_CMD_ID:
		...
		if(BindRspCluterId == MOTOR_CONTROL_CLUSTERID)
		{
		  	Bind_Clean(MOTOR_CONTROL_CLUSTERID);
