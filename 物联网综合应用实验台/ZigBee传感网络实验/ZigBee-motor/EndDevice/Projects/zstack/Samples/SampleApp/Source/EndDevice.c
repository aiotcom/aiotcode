...
uint16 SampleApp_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  {
	ResponeBind(MOTOR_CONTROL_CLUSTERID,&RspAddr);
...
{ 
  switch ( pkt->clusterId )
  {
    case BIND_REQUEST_CMD_ID:
		 if(inBindList((const cId_t*)pkt->cmd.Data,pkt->cmd.DataLength/2,MOTOR_CONTROL_CLUSTERID))
		 {
		    printf("get bind cmd\r\n");
	...
	case MOTOR_CONTROL_CLUSTERID://接收到控制指令
		printf("motor cmd\r\n");
