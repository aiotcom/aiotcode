...
uint16 SampleApp_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  {
	ResponeBind(SIREN_CONTROL_CLUSTERID,&RspAddr);
...
{ 
  switch ( pkt->clusterId )
  {
    case BIND_REQUEST_CMD_ID:
		 if(inBindList((const cId_t*)pkt->cmd.Data,pkt->cmd.DataLength/2,SIREN_CONTROL_CLUSTERID))
		 {
		    printf("get bind cmd\r\n");
	...
	case SIREN_CONTROL_CLUSTERID://接收到控制指令
		printf("siren cmd\r\n");
		SirenCmd = pkt->cmd.Data[0] + (pkt->cmd.Data[1]<<8);
		osal_set_event( RS485_Handler_TaskId, SEND_SIREN_CONTROL_EVENT );
	  	break;
  }
}
/*
