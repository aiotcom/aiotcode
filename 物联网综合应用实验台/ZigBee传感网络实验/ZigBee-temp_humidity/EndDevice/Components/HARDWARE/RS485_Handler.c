...
uint16 RS485_Handler_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if ( (events & RS485_HANDLER_MSG_EVT) == RS485_HANDLER_MSG_EVT )
  {
	if(state == 0)
	{
		Rs485_Send(Addr_NULL,Addr_WS,Get_WS_Data,0,(void*)0);
		count = 0;
		state = 1;
...
#if 0  // for my debugger
		if(1)
#else
		if(!DataHandling_485(Addr_NULL))
#endif
		{/*高字节在前*/
		  	/*温度2byte+湿度2byte*/
		  	sensor_data[0] = Rx_Stack.Data[0];
		  	sensor_data[1] = Rx_Stack.Data[1];		
		  	sensor_data[2] = Rx_Stack.Data[2];
		  	sensor_data[3] = Rx_Stack.Data[3];			
