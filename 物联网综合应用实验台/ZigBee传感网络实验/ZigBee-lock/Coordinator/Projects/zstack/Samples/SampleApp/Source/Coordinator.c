...
void Coor_Send_Lock_ControlCmd(uint16 cmd);
/*********************************************************************
 * @fn      SampleApp_Init
 *
...
uint16 SampleApp_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  {//检测到按键按下，发送指令
	...
	if(KeyValue == KEY_S1)
	{//按键S1->开锁指令
		Coor_Send_Lock_ControlCmd(Lock_OFF);
	}
	else if(KeyValue == KEY_S2)
	{//按键S2->关锁指令
		Coor_Send_Lock_ControlCmd(Lock_ON);
	}
...
{ 
  ...
  case BIND_RSP_CMD_ID:
		...
		printf("==%04X",BindRspCluterId);
		 if(BindRspCluterId == LOCK_CONTROL_CLUSTERID)
		{
