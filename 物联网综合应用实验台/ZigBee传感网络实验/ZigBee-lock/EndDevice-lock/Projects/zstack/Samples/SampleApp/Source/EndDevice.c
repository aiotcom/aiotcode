...
uint16 SampleApp_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  {
	ResponeBind(LOCK_CONTROL_CLUSTERID,&RspAddr);
...
{ 
  ...
  switch ( pkt->clusterId )
  {
    case BIND_REQUEST_CMD_ID:
		 if(inBindList((const cId_t*)pkt->cmd.Data,pkt->cmd.DataLength/2,LOCK_CONTROL_CLUSTERID))
		 {
		    printf("get bind cmd\r\n");
	...
	case LOCK_CONTROL_CLUSTERID://接收到控制指令
		printf("lock cmd\r\n");
		LockCmd = pkt->cmd.Data[0] + (pkt->cmd.Data[1]<<8);
		osal_set_event( RS485_Handler_TaskId, SEND_LOCK_CONTROL_EVENT );
	  	break;
  }
}
/*
