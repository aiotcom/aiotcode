...
void SendSensorDataToCoordinator(uint8_t *sdata);
/*********************************************************************
 * GLOBAL VARIABLES
 */
...
uint16 SampleApp_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if((events & BIND_RSP_EVENT) == BIND_RSP_EVENT)
  {
	ResponeBind(CURTAINS_CONTROL_CLUSTERID,&RspAddr);
	return (events ^ BIND_RSP_EVENT);
  }
...
{ 
  switch ( pkt->clusterId )
  {
    case BIND_REQUEST_CMD_ID:
		 if(inBindList((const cId_t*)pkt->cmd.Data,pkt->cmd.DataLength/2,CURTAINS_CONTROL_CLUSTERID))
		 {
		    printf("get bind cmd\r\n");
			osal_start_timerEx( SampleApp_TaskID,BIND_RSP_EVENT,(rand()%10)*10);
			RspAddr = pkt->srcAddr;
		 }
      break;
	  
	case CURTAINS_CONTROL_CLUSTERID://接收到控制指令
		printf("motor cmd\r\n");
		MotorCmd = pkt->cmd.Data[0] + (pkt->cmd.Data[1]<<8);
		osal_set_event( RS485_Handler_TaskId, SEND_MOTOR_CONTROL_EVENT );
	  	break;
  }
}
/*
