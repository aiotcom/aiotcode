...
uint16 Hal_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if((events & READ_KEY_EVT) == READ_KEY_EVT)
  {
	osal_start_timerEx( Hal_TaskID, READ_KEY_EVT,100);
	KeyValue = key_handler();
	if(KeyValue)
	{
		osal_set_event( SampleApp_TaskID, GET_KEY_EVT );
	}
  	return events ^ READ_KEY_EVT;
  }	
