...
void Coor_Send_FAN_ControlCmd(uint16 cmd);
/*********************************************************************
 * @fn      SampleApp_Init
 *
...
void SampleApp_Init( uint8 task_id )
{
  ...
  printf("i am coordinator for light intensity\r\n");//串口打印
  OLED_P8x16Str(0,0,"coor-Fan");
...
uint16 SampleApp_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  {//检测到按键按下，发送指令
	...
	if(KeyValue == KEY_S1)
	{//按键S1->风扇关指令
		Coor_Send_FAN_ControlCmd(Fan_OFF);
	}
	else if(KeyValue == KEY_S2)
	{//按键S2->风扇开指令
		Coor_Send_FAN_ControlCmd(Fan_ON);
	}
...
#define TEMP_HUMI_CLUSTERID          0x0004 
float value;
...
{ 
  ...
  case BIND_RSP_CMD_ID:
		...
		if(BindRspCluterId == FAN_CONTROL_CLUSTERID)
		{
		  	Bind_Clean(FAN_CONTROL_CLUSTERID);
			Point_To_Point_DstAddr.addr.shortAddr = pkt->srcAddr.addr.shortAddr;
			OLED_P8x16Str(0,0,"binding ");
		}
		else if(BindRspCluterId == LOCK_CONTROL_CLUSTERID)
		{
		  //	Bind_Clean(LOCK_CONTROL_CLUSTERID);
	...
	case TEMP_HUMI_CLUSTERID:
	  	printf("==%d,%d,%d,%d\r\n",pkt->cmd.Data[0],pkt->cmd.Data[1],pkt->cmd.Data[2],pkt->cmd.Data[3]);
		memset((void*)OLED_DisplayBuf,0,32);
		value = ((pkt->cmd.Data[0]<<8)+pkt->cmd.Data[1]);
		value = value/10;		
		sprintf((void*)OLED_DisplayBuf,"humi=%.1f%%",value);
		OLED_P8x16Str(2,2,OLED_DisplayBuf);

		memset((void*)OLED_DisplayBuf,0,32);	
		value = (pkt->cmd.Data[2]<<8)+pkt->cmd.Data[3];
		value = value/10;			
		sprintf((void*)OLED_DisplayBuf,"temp=%.1fdegC",value);
		OLED_P8x16Str(2,4,OLED_DisplayBuf);
