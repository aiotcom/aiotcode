...
uint16 RS485_Handler_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if( (events & SEND_FAN_CONTROL_EVENT) == SEND_FAN_CONTROL_EVENT)
  {
	printf("snd fan cmd%d\r\n",FanCmd);
  	if(FanCmd == 0)
	{
		Rs485_Send(Addr_NULL,Addr_Fan,Fan_OFF,0,0);
	}
	else
	{
		Rs485_Send(Addr_NULL,Addr_Fan,Fan_ON,0,0);	
	}
	return events ^ SEND_FAN_CONTROL_EVENT;
  }  
  return 0;
}
