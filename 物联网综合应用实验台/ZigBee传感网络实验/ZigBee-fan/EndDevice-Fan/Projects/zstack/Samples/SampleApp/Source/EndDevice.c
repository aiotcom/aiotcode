...
uint16 SampleApp_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  {
	ResponeBind(FAN_CONTROL_CLUSTERID,&RspAddr);
...
{ 
  switch ( pkt->clusterId )
  {
    case BIND_REQUEST_CMD_ID:
		 if(inBindList((const cId_t*)pkt->cmd.Data,pkt->cmd.DataLength/2,FAN_CONTROL_CLUSTERID))
		 {
		    printf("get bind cmd\r\n");
	...
	case FAN_CONTROL_CLUSTERID:
		printf("fan cmd\r\n");
		FanCmd = pkt->cmd.Data[0];
		osal_set_event( RS485_Handler_TaskId, SEND_FAN_CONTROL_EVENT );
	  	break;
  }
}
/*
