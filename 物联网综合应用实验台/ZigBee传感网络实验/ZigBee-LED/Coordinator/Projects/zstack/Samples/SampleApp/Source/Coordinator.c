...
void Coor_Send_LED_ControlCmd(uint16 cmd);
/*********************************************************************
 * @fn      SampleApp_Init
 *
...
void SampleApp_Init( uint8 task_id )
{
  ...
  printf("i am coordinator for LED\r\n");//串口打印
  OLED_P8x16Str(0,0,"coor-LED");
...
uint16 SampleApp_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  {//检测到按键按下，发送指令
	...
	if(KeyValue == KEY_S1)
	{//按键S1->控制LED1
	  Coor_Send_LED_ControlCmd(LedState[0]?LED_ON1:LED_OFF1);
	  LedState[0] = 1 - LedState[0];
	}
	else if(KeyValue == KEY_S2)
	{//按键S2->控制LED2
	  Coor_Send_LED_ControlCmd(LedState[1]?LED_ON2:LED_OFF2);
	  LedState[1] = 1 - LedState[1];
	}
	else if(KeyValue == KEY_S3)
	{//按键S1->控制LED3
	  Coor_Send_LED_ControlCmd(LedState[2]?LED_ON3:LED_OFF3);
	  LedState[2] = 1 - LedState[2];
	}
...
{ 
  ...
  case BIND_RSP_CMD_ID:
		...
		if(BindRspCluterId == LED_CONTROL_CLUSTERID)
		{
		  	Bind_Clean(LED_CONTROL_CLUSTERID);
