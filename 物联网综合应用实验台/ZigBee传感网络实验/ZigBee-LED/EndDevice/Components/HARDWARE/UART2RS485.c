...
*/
	...
	{
		if(RS485_RX_STA>(Receive_REC_LEN-1))
		{
			End_Flag = 0;
			Receive_Flag = 0;
			RS485_RX_STA = 0;
		...
		Receive_Data[RS485_RX_STA++] = data;	//数据接收
		...
		{
			RS485_RX_STA = Receiving_Completion;		//接收完成
			Receive_Flag = 0;
			End_Flag = 0;
		}
...
uint8_t DataHandling_485(uint16_t DesAddr)
{
	...
	static uint16_t Crc16Check = 0;			//存放CRC校验数据
  	...
  	if(RS485_RX_STA == Receiving_Completion)
	{
			Return_Value = 0;
...
//==========================================================
//	函数名称：	void Rs485_ClearReceiveBuffer(void)
//
//	函数功能：	清除接收缓冲
//
//	入口参数：	无
...
//	说明：		
//==========================================================
void Rs485_ClearReceiveBuffer(void)
{
    RS485_RX_STA = 0;
