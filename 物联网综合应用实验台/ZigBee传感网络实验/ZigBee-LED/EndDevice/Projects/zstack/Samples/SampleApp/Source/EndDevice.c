...
uint16 SampleApp_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  {
	ResponeBind(LED_CONTROL_CLUSTERID,&RspAddr);
...
{ 
  switch ( pkt->clusterId )
  {
    case BIND_REQUEST_CMD_ID:
		 if(inBindList((const cId_t*)pkt->cmd.Data,pkt->cmd.DataLength/2,LED_CONTROL_CLUSTERID))
		 {
		    printf("get bind cmd\r\n");
	...
	case LED_CONTROL_CLUSTERID:
		printf("LED cmd\r\n");
		LedCmd = pkt->cmd.Data[0] + (pkt->cmd.Data[1]<<8);
		osal_set_event( RS485_Handler_TaskId, SEND_CONTROL_EVENT );
	  	break;
  }
}
/*
