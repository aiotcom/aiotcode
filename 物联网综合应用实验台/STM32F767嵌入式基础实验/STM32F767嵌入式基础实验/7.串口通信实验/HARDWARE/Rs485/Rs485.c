...
void Rs485_Send(uint16_t SrcAdr,uint16_t DstAdr,uint16_t Cmd,uint8_t Len,uint8_t *pData)
{
	...
	RS485_TX_EN();		//开启485发送
	Rs485_delay_ms(5);
	HAL_UART_Transmit(&UART7_Handler,Send_Data,(18+Len),1000);//485发送数据
	Rs485_delay_ms(5);
	RS485_RX_EN();		//开启485接收
}
...
*/
	...
	{
		if(USART7_RX_STA>(Receive_REC_LEN-1))
		{
			End_Flag = 0;
			Receive_Flag = 0;
			USART7_RX_STA = 0;
		...
		Receive_Data[USART7_RX_STA++] = data;	//数据接收
		...
		{
			USART7_RX_STA = Receiving_Completion;		//接收完成
			Receive_Flag = 0;
			End_Flag = 0;
		}
...
uint8_t DataHandling_485(uint16_t DesAddr)
{
  ...
  if(USART7_RX_STA == Receiving_Completion)
	{
			Return_Value = 0;
