...
static void spi_init(void)
{
	...
	PA2_GPIO_PAD     =   (unsigned int*)ioremap(0x20E053C,4);
	...
	PA3_GPIO_PAD     =  (unsigned int*)ioremap(0x20E0540,4);
	
	imx6_gpio_mode(PA2_MUX);
	imx6_gpio_mode(PA3_MUX);	
	
	imx6_gpio_in(PA2_GPIO_GDIR,PA2_GPIO_NBR);
	imx6_gpio_pupd(PA2_GPIO_PAD,PD_100K_OHM);
	imx6_gpio_in(PA3_GPIO_GDIR,PA3_GPIO_NBR);
	imx6_gpio_pupd(PA3_GPIO_PAD,PD_100K_OHM);	
}
/*************************************************/
...
static int key_read(struct file *filp, char __user *buff,size_t count, loff_t *offp)
{
	...
	byte[0] = !imx6_gpio_read(PA3_GPIO_DR,PA3_GPIO_NBR);
	byte[1] = !imx6_gpio_read(PA2_GPIO_DR,PA2_GPIO_NBR);
	err  = copy_to_user(buff, (const void *)&byte[0], count);
    return err ? -EFAULT : 1;
}
...
static int __init button_init(void)
{
	...
	printk("pt333 device installed, with major %d minor %d\r\n", key_major,key_minor);	
	if (devno < 0) {
		printk(KERN_WARNING "pt333: unable to get major %d\n", key_major);
		return result;
	}
