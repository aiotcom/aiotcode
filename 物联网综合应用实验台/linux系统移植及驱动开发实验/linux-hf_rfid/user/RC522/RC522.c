...
#define rc522_MAGIC 'k'
#define IOCTRL_CMD_CS _IOW(rc522_MAGIC, 1, int)
#define IOCTRL_CMD_RST _IOW(rc522_MAGIC, 2, int)
const unsigned int arg_0=0,arg_1=1;
int rc522_fd;
//==========================================================
//	函数名称：	void macRC522_Reset_Disable()
//
//	函数功能：	停步复位
//
//	入口参数：	无
...
//	说明：		
//==========================================================
void macRC522_Reset_Disable(void)
{
	ioctl(rc522_fd, IOCTRL_CMD_RST, &arg_1);		
}
//==========================================================
//	函数名称：	void macRC522_Reset_Enable()
//
//	函数功能：	使能复位
//
//	入口参数：	无
...
//	说明：		
//==========================================================	
void macRC522_Reset_Enable(void)
{
	ioctl(rc522_fd, IOCTRL_CMD_RST, &arg_0);		
}
//==========================================================
//	函数名称：	void macRC522_CS_Disable()
//
//	函数功能：	停止片选
//
//	入口参数：	无
...
//	说明：		
//==========================================================
void macRC522_CS_Disable(void)
{
	ioctl(rc522_fd, IOCTRL_CMD_CS, &arg_1);	
}
//==========================================================
//	函数名称：	void macRC522_CS_Enable(void)
//
//	函数功能：	使能片选
//
//	入口参数：	无
...
//	说明：		
//==========================================================
void macRC522_CS_Enable(void)
{
	ioctl(rc522_fd, IOCTRL_CMD_CS, &arg_0);	
}
//==========================================================
...
void RC522_Init(void)
{
	/*打开键盘设备文件*/
	printf("rc522 init\r\n");
	rc522_fd = open("/dev/rc522", O_RDWR);
	if (rc522_fd < 0) {
		perror("**/dev/rc522\r\n");
		exit(1);
	}
...
unsigned char IC_Card_Search ( unsigned char *pid )
{
    char cStr [ 30 ];
    unsigned char i=0;
...
unsigned char IC_Card_Write(unsigned char *p)
{
    if (PcdAnticoll(ucArray_ID) == MI_OK)  //防冲撞（当有多张卡进入读写器操作范围时，防冲突机制会从其中选择一张进行操作）
    {               
        if (PcdSelect(ucArray_ID) == MI_OK)//选择ucArray_ID的卡
...
//	说明：		
//==========================================================
void IC_Card_Read(unsigned char *p)
{
     PcdRead(0X5,p);
}
//==========================================================
