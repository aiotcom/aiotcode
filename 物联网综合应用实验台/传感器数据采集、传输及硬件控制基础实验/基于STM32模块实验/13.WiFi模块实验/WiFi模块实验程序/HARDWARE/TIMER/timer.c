...
uint32_t Time_Count = 0,Send_OneNET_Count = 0;//分时任务计数值
...
{
	Time_Count>99999?(Time_Count = 0):(Time_Count++);
	Send_OneNET_Count>99999?(Send_OneNET_Count = 0):(Send_OneNET_Count++);

	WiFi_KEY_Read()?(WiFi_RST_High()):(WiFi_RST_Low());	//检测按键
