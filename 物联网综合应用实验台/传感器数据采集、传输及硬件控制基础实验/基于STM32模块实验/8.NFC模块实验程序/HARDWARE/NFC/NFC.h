...
void PN532_Init(void);												//初始化PN532
void  SendData(u8 *data,u8 length); 					//往USART1，发送 length长度的数据data
void  CleanBuffer(u16 num);									 	//清零 前 多少个字节的数据

u8 	SAMConfiguration(u8 mode,u8 timeout,u8 irq); 			 					//配置参数
u8  nfc_InListPassiveTarget(u8 maxTag,u8 brty,u8 *CardID); 						//寻卡
u8  nfc_PsdVerify(u8 AuthenticationType,u8 BlockNo,u8 * KeyData,u8 *SerialNum); //块验证密钥    
u8  nfc_read(u8 BlockNO,u8 *BlockData);											//读块
u8  nfc_write(u8 BlockNO,u8 *WriteData);										//写块
u8 	nfc_getIdentityID(u8 *identityid);											//读取身份证
u8 SPI1_ReadWriteByte(u8 TxData);
extern u8 UID[4];		   
extern u8 flag_nfc_status;
extern u8 UID_backup[4];
extern u8 KeyBuf[6];
extern u8 BlockBuf[16];
