					...
					if (PcdSelect(ucArray_ID) == MI_OK)//选择ucArray_ID的卡
					{
							...
							if (PcdAuthState(0x61,0x5,pk,ucArray_ID) == MI_OK)//申请对卡的授权  
							{
									if (PcdRead(0x5,date1) == MI_OK)
									{
										...
										Rs485_Send("ID:",3);
										Rs485_Send(SendData,8);
										Rs485_Send("\r\n",2);
										
										Rs485_Send("Block5_Date:",12);
										Rs485_Send(SendBuf,16);
										Rs485_Send("\r\n",2);
									}
							}
					}
...
{     
	...
	GPIO_Initure.Pin=RC522_LED_PIN|RC522_CS_PIN|RC522_SCK_PIN|RC522_MOSI_PIN;
	GPIO_Initure.Mode=GPIO_MODE_OUTPUT_PP;  	//推挽输出
	GPIO_Initure.Speed=GPIO_SPEED_HIGH;    	 	//高速
	HAL_GPIO_Init(GPIOA,&GPIO_Initure);
	
	GPIO_Initure.Pin=RC522_MISO_PIN;
	GPIO_Initure.Mode=GPIO_MODE_INPUT;  	    //输入模式
	GPIO_Initure.Speed=GPIO_SPEED_HIGH;    	 	//高速
	HAL_GPIO_Init(RC522_MISO_PORT,&GPIO_Initure);      

	GPIO_Initure.Pin=RC522_Reset_PIN;
	GPIO_Initure.Mode=GPIO_MODE_OUTPUT_PP;  	//推挽输出
	GPIO_Initure.Speed=GPIO_SPEED_HIGH;    	 	//高速
	HAL_GPIO_Init(RC522_Reset_PORT,&GPIO_Initure);
	macRC522_LED_OFF();
}
