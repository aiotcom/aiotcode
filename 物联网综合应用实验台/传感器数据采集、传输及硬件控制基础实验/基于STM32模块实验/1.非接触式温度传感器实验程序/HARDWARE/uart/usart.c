...
{	
  ...
  UART7_Handler.Init.OverSampling = UART_OVERSAMPLING_16;
	...
	Rs485_Send("欢迎使用蜂巢套件-非接触温度传感器模块实验\r\n",41);
}
...
//	说明：			
//==========================================================
void USART7_IRQHand(void)
{
	static uint8_t len =0;			//存放数据长度
	
	if(USART7_RX_STA&0x8000)
	{
		len=USART7_RX_STA&0x3fff;//得到此次接收到的数据长度	
		...
		RS485_TX_EN();
		delay_ms(5);
		HAL_UART_Transmit(&UART7_Handler,(uint8_t*)USART7_RX_BUF,len,1000);	//发送接收到的数据
		delay_ms(5);
		RS485_RX_EN();
		...
		USART7_RX_STA = 0;
		memset(USART7_RX_BUF,0,strlen((char *)USART7_RX_BUF));
	}
}

//==========================================================
//	函数名称：	USART7_Data_Send
//
//	函数功能：	串口七数据发送
//
//	入口参数：	无
...
//	说明：			
//==========================================================
void USART7_Data_Send(void)
{
	static uint8_t key = 0,KEY_DATA[30];
	...
	switch(key)
	{
		case 1:
			...
			sprintf((char *)KEY_DATA,"按键S1按下\r\n");
			break;
		case 2:
			HAL_GPIO_TogglePin(LED_PORT,LED2_PIN);
			sprintf((char *)KEY_DATA,"按键S2按下\r\n");
			break;
		case 3:
			HAL_GPIO_TogglePin(LED_PORT,LED3_PIN);
			sprintf((char *)KEY_DATA,"按键S3按下\r\n");
			break;
		case 4:
			HAL_GPIO_TogglePin(LED_PORT,LED4_PIN);
			sprintf((char *)KEY_DATA,"按键S4按下\r\n");
			break;
		default:break;
	}
	...
	if(key)
	{
		RS485_TX_EN();
		delay_ms(5);
		HAL_UART_Transmit(&UART7_Handler,KEY_DATA,30,1000);	//发送按键值
		delay_ms(5);
		RS485_RX_EN();
	}
}
