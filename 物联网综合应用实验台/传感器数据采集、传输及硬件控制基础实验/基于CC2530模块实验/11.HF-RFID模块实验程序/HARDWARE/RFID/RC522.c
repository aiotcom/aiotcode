                ...
                if (PcdAuthState(0x61,0x5,pk,ucArray_ID) == MI_OK)//申请对卡的授权  
                {
                    if (PcdRead(0x5,Block_Date) == MI_OK )
                    {
                        ...
                        for(i = 0;i < 32;i ++)
                        {
                          if(i%2==0)
                          {
                             DataRep((Block_Date[(i/2)]>>4),i,1,0);
                          }
                          else
                          { 
                            DataRep((Block_Date[(i/2)]&0x0f),i,0,0);
                          }      
                        }
                        OLED_P8x16Str(28,2,"0x");
                        OLED_P8x16Str(46,2,SendData);
                        OLED_P6x8Str(40,6,"0x");
                        OLED_P6x8Str(55,6,SendBuf);
                    }
                }
            }
        }
