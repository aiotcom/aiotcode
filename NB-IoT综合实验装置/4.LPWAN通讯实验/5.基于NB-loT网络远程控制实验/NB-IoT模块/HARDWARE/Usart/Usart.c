...
uint16_t Command_LED,Command_Relay,Command;	//控制LED、继电器命令码
...
void USART_Data_Send(void)
{
	...
	if(Task_Count>=10 && Send_Flag == 0)				//100ms		获取温湿度
	{
		Rs485_Send(Addr_NB,Addr_SHT20,SHT20_Get_All,0,"");
		Send_Flag = 1;
	}
	else if(Task_Count>=30 && Send_Flag == 1)		//300ms 	控制LED灯
	{
		Rs485_Send(Addr_NB,Addr_LED,Command_LED,0,"");
		Send_Flag = 2;
	}
	else if(Task_Count>=50 && Send_Flag == 2)		//500ms		控制继电器
	{
