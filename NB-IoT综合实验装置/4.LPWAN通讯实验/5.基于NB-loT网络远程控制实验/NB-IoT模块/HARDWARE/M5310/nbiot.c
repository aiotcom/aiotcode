...
uint8_t ReadRspRelay[]={"+MIPLREAD:0,62551,3300,0,5701,1,\"123\",1\r\n"};

nbiot_uri_t temp_uri;
nbiot_uri_t humi_uri;
nbiot_uri_t illum_uri;
nbiot_uri_t led_uri;
nbiot_uri_t relay_uri;
...
//***************************LED**********************************************>>>  
   delay_ms(1000);
//***************************继电器**********************************************<<< 
    m5310_addobj(OBJ_Relay_ID,INST_ID_Relay);             //添加对象object 继电器
		relay_uri.objid  = OBJ_Relay_ID;
		relay_uri.instid = INST_ID_Relay;
		relay_uri.resid  = RES_ID_Relay;
		relay_uri.flag  |= NBIOT_SET_OBJID;
		relay_uri.flag  |= NBIOT_SET_INSTID;
    m5310_notify_upload(&relay_uri,NBIOT_STRING,"5",0,0); 	//订阅资源    
//***************************继电器**********************************************>>>  
		delay_ms(1000);
}
...
void handle_request( nbiot_device_t *dev,uint16_t code, uint8_t *buffer,size_t buffer_len,size_t max_buffer_len )
{
	...
	if(Command>=0x1110 && Command<=0x1141)
	{
		Command_LED = Command;
	}
	else if(Command>=0x1410 && Command<=0x1421)
	{
	...
	if( COAP_READ == code )
	{
		...
		memcpy((void*)&ReadRspRelay[12],(const void*)&RspMsgID[0],5);
		AT_SendCmd((char*)ReadRspRelay, "OK",0,500);
