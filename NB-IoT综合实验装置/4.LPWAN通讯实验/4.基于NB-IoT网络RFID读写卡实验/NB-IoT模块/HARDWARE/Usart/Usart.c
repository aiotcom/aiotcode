...
{
	static uint8_t Send_Flag = 0,i = 0;
	
	if(Task_Count>=100 && Send_Flag == 0)				//100ms		获取RFID数据
	{
		Rs485_Send(Addr_NB,Addr_HF_RFID,HF_RFID_Get_All,0,"");
		Send_Flag = 1;
	}
	else if(Task_Count>=300 && Send_Flag == 1)	//300ms 	发送数据到显示器显示
	{
		Rs485_Send(Addr_NB,Addr_TFT,TFT_Display,20,Rx_Stack.Data);	
		Send_Flag = 0;
		Task_Count = 0;
	}
...
void USART1_IRQHand(void)
{
	...
	{
		if(Rx_Stack.Src_Adr == Addr_HF_RFID)//处理RFID数据
		{
			for(i=0;i<4;i++)
			{
