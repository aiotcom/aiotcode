...
void USART_Data_Send(void)
{
	...
	if(Task_Count>=200 && Send_Flag == 0)				//200ms		获取PM2.5数据
	{
		Rs485_Send(Addr_WiFi,Addr_PM2_5,PM2_5_Get_Conc,0,0);
		Send_Flag = 1;
	}
	else if(Task_Count>=400 && Send_Flag == 1)	//400ms 	获取大气压力数据
	{
		Rs485_Send(Addr_WiFi,Addr_A_P,A_P_Get_Altitude,0,0);
		Send_Flag = 2;
	}
	else if(Task_Count>=600 && Send_Flag == 2)	//600ms 	获取温湿度数据
	{
		Rs485_Send(Addr_WiFi,Addr_SHT20,SHT20_Get_All,0,0);
		Send_Flag = 3;
	}
	else if(Task_Count>=800 && Send_Flag == 3)	//800ms 	获取火焰传感器状态
	{
		Rs485_Send(Addr_WiFi,Addr_Flame,Flame_Get_State,0,0);
		Send_Flag = 4;
	}
	else if(Task_Count>=1000 && Send_Flag == 4)	//1000ms 	获取人体红外状态
	{
		Rs485_Send(Addr_WiFi,Addr_PIR,PIR_Get_State,0,0);
		Send_Flag = 5;
	}
	else if(Task_Count>=1200 && Send_Flag == 5)	//1200ms 	获取烟雾传感器数据
	{
		Rs485_Send(Addr_WiFi,Addr_Smoke,Smoke_Get_State,0,0);
		Send_Flag = 6;
	}
	else if(Task_Count>=1400 && Send_Flag == 6)	//1400ms 	获取震动传感器状态
	{
		Rs485_Send(Addr_WiFi,Addr_Shock,Shock_Get_Count,0,0);
		Send_Flag = 0;
		Task_Count = 0;
	}

	if(Send_OneNET_Count>=550)
	{
		sprintf((char *)Send_OneNET,"Sensor:%d,%d,%d,%d,%d,%d,%d,%d,%d",Shock_Sta,PIR_Sta,Flame_Sta,
			Smoke_Value,A_P_Data,PM2_5_Data,Temp,Humi,LDR_Data);
		HAL_UART_Transmit(&UART2_Handler,Send_OneNET,35,1000);	//发送数据到云平台
		memset(Send_OneNET,0x30,35);
		Send_OneNET_Count = 0;
	}
}
...
void USART1_IRQHand(void)
{
	if(!DataHandling_485(Addr_WiFi))			//485数据处理
	{
		...
		{
			case Addr_Shock:	//震动传感器模块数据
				Shock_Sta = Rx_Stack.Data[0];		//震动传感器状态
				break;
			case Addr_PIR:		//人体红外模块数据
				PIR_Sta = Rx_Stack.Data[0];			//人体红外传感器状态
				break;
			case Addr_Flame:	//火焰传感器模块数据
				Flame_Sta = Rx_Stack.Data[0];		//火焰感器状态
				break;
			case Addr_Smoke:	//烟雾传感器模块数据
				Smoke_Value = Rx_Stack.Data[0];	//烟雾传感器数据
				break;
			case Addr_A_P:		//大气压力模块数据
				A_P_Data = Rx_Stack.Data[0]*256+Rx_Stack.Data[1];		//大气压力数据
				break;
			case Addr_PM2_5:	//PM2.5模块数据
				PM2_5_Data = Rx_Stack.Data[0]*256+Rx_Stack.Data[1];	//PM2.5数据
				break;
			case Addr_SHT20:	//温湿度模块数据
