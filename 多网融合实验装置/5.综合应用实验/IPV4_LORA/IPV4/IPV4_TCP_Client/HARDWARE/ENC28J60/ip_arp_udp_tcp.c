...
unsigned  int checksum(unsigned char *buf, unsigned  int len,unsigned char type)
{
    ...
    if(type==1)
    {
        sum+=IP_PROTO_UDP_V; // protocol udp 17
        // the length here is the length of udp (data+header len)
        // =length given to this function - (IP.scr+IP.dst length)源地址+目的地址长度4+4
        sum+=len-8; // = real udp len
    }
    if(type==2)
    {
        sum+=IP_PROTO_TCP_V; //6
    ...
    return( (unsigned  int) sum ^ 0xFFFF);	  //返回校验值
}

//配置本机的www端口，IP地址，mac地址
...
unsigned char eth_type_is_ip_and_my_ip(unsigned char *buf,unsigned  int len)
{
    ...
    {
        //一般为0x45, 4代表ip版本4,5代表结构体长度,就是IP_HEADER的长度,为20字节,这里只支持20字节的
