...
int main(void)
{
	...
	while(1)
	{
        ...
        {//接收到数据
            SendCount = HAL_GetTick()+1000;
            if(ConndecServerFlg)
            {
                sprintf((void*)uip_tcp_client_send_buf, 
                        "temp->%d,humi->%d,LightIntensity->%d\r\n",
                         TxBuffer[0],TxBuffer[1],((TxBuffer[3]<<8)|TxBuffer[2]));
...
{  
    ...
    if(state == 0)
    {
        ...
        {//2000ms发送一次轮询指令
            PollCounter = 0;
            Rs485_Send(Addr_IPV4,Addr_LORA,SHT20_Get_All,  0,(void*)0); 
            state = 1;
        }
    }
    else if(state == 1)
