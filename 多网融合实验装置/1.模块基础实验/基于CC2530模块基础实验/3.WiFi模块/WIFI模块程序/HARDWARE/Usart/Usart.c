...
void USART1_IRQHand(void)
{
  if(!DataHandling_485(Addr_WiFi))	//485数据处理
  {
    if(Rx_Stack.Src_Adr == Addr_SHT20)	//获取温湿度数据
    {
	  ...
	  Light_intensity = (Rx_Stack.Data[3]<<8) +Rx_Stack.Data[2];
    }
  }
}
...
void USART_Data_Send(void)
{
  ...
  {
    sprintf((char *)Send_OneNET,"%d%d%04d",Temp,Humi,Light_intensity);
    USART0_SendString(Send_OneNET,8);//发送数据到OneNET
    Send_OneNET_Count = 0;
  }
}
