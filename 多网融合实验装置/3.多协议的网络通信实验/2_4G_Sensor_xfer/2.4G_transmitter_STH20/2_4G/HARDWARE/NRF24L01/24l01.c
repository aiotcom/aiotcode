...
void NRF24L01_RX_Mode(uint8_t rfch)
{
    NRF24L01_CE(0);	  
    ...
    NRF24L01_Write_Reg(NRF_WRITE_REG+RF_CH,rfch);	    	//设置RF通信频率		  
...
//当CE变高后,即进入RX模式,并可以接收数据了		   
//CE为高大于10us,则启动发送.	 
void NRF24L01_TX_Mode(uint8_t rfch)
{
    NRF24L01_CE(0);	    
    ...
    NRF24L01_Write_Reg(NRF_WRITE_REG+RF_CH,rfch);       //设置RF通道为40
    NRF24L01_Write_Reg(NRF_WRITE_REG+RF_SETUP,0x26);  //设置TX发射参数,0db增益,250Kbps,低噪声增益开启   
