...
int icd  = 0;
uint8 i = 0;
uint8 IC_Card_Exist = 0;
unsigned char pk[6] ={0xff,0xff,0xff,0xff,0xff,0xff};     
unsigned char ucArray_ID [ 16 ] = {0,0,0,0};//先后存放IC卡的类型和UID(IC卡序列号)
uint8 SendBuf[33];
uint8 SendData[9];
...
//	说明：		
//==========================================================
void DataRep(uint8 data,uint8 num,uint8 flag,uint8 flg)
{
  if(flg)
  {
    switch(data)
