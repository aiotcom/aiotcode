...
void npSpiMonitor(void)
{
  ...
  if (npSpiState == NP_SPI_IDLE)
  {
    *((uint8 *)DMA_UDBUF) = 0;  // Clear the SPI Tx buffer to zero.
    ...
    (void)osal_stop_timerEx(znpTaskId, ZNP_SPI_TIMER_EVENT);
    (void)osal_clear_event(znpTaskId, ZNP_SPI_TIMER_EVENT);
  }
  else
  {
    (void)osal_start_timerEx(znpTaskId, ZNP_SPI_TIMER_EVENT, ZNP_SPI_TIMER_DELAY);
  }
...
{
  if (npSpiState == NP_SPI_WAIT_TX)
  {
    //NP_SPI_ASSERT(len <= NP_SPI_BUF_LEN);
    return npSpiBuf;
  }
  else
  {
...
{
  if ((npSpiState == NP_SPI_WAIT_TX) && (NP_RDYOut == 0))
  {
    DMA_TX( pBuf );
    NP_RDYOut = 1;
  }
