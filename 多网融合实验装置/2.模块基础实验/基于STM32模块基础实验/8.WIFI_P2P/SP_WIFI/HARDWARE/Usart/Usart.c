	...
	if(USART_AT_STA)																									//AT指令发送完成
	{
		if(USART1_RX_STA&0x8000)
		{
			...
			{	
				...
				delay_ms(300);
				HAL_UART_Transmit(&UART2_Handler,"wif->wif",8,1000);					//串口2发送数据
				HAL_UART_Transmit(&UART2_Handler,(uint8_t *)&number1,1,1000);	//串口2发送数据
				HAL_UART_Transmit(&UART2_Handler,"bc",2,1000);								//串口2发送数据
			}
			USART1_RX_STA=0;
...
void USART2_IRQHand(void)
{
	...
	if(USART_AT_STA)	//AT指令发送完成
	{
		if(USART2_RX_STA&0x8000)
		{
			...
			delay_ms(100);											//延时100ms使数据稳定
			Rs485_Config(1);   									//设置485为发送模式
			HAL_UART_Transmit(&UART1_Handler,"wif->led",8,1000);				//串口1发送数据
			HAL_UART_Transmit(&UART1_Handler,(uint8_t *)&number2,1,1000);//串口1发送数据
			HAL_UART_Transmit(&UART1_Handler,"\r\n",2,1000);						//串口1发送数据
			delay_ms(10);
			Rs485_Config(0);   									//设置485为接收模式
