...
uint8_t LoraRecvBuffer[32],UpdataFlg=0,TxBuffer[20];
void RS485_HandlerCb(void);//处理485总线数据
#define RF_Frequency 500 //范围 410~525(单位M)
int main(void)
{
	uint8_t LedState = 1;
	...
	while(1)
	{
        ...
        if((IRQ_RegValue)&&(IRQ_RegValue != 0XFF))//读 LORA中断寄存器 REG_LR_IRQFLAGS，判断是否有中断发生
        {
            ...
            {//是接收中断，将接收的数据保存到LoraRecvBuffer中
                TxBuffer[0] = LoraRecvBuffer[0];
                TxBuffer[1] = LoraRecvBuffer[1];
                TxBuffer[2] = LoraRecvBuffer[2];
                TxBuffer[3] = LoraRecvBuffer[3];   
                printf("%d,%d,%d\r\n",TxBuffer[0],TxBuffer[1],((TxBuffer[3]<<8)|TxBuffer[2])); //调试信息               
            }
        }         
	}
}
...
void RS485_HandlerCb(void)
{
    ...
    {
        Rs485_Send(Addr_LORA,Addr_IPV4,SHT20_All,4,TxBuffer);
    }
}
