...
#define  COORDINATOR_SWITCH_PERIOD_TIMEOUT          5000
#define  COORDINATOR_SWITCH_NETGROUNP_MSG_EVT       0x0001
/*********************************************************************
 * GLOBAL VARIABLES
 */
...
const cId_t SampleApp_InClusterList[2] =
{
...
//***************设置组播通信目的地址参数******************************     
  Group_DstAddr.addr.shortAddr = SAMPLE_APP_GROUNP; 
  SampleApp_Group.ID = SAMPLE_APP_GROUNP;
...
uint16 SampleApp_ProcessEvent( uint8 task_id, uint16 events )
{
        ...
        case ZDO_STATE_CHANGE:
          ...
          if(SampleApp_NwkState == DEV_ZB_COORD)
          {
            ...
            osal_start_timerEx( SampleApp_TaskID,
                                COORDINATOR_SWITCH_NETGROUNP_MSG_EVT,
                                COORDINATOR_SWITCH_PERIOD_TIMEOUT );   
          }
          break;
  ...
  if((events&COORDINATOR_SWITCH_NETGROUNP_MSG_EVT) == COORDINATOR_SWITCH_NETGROUNP_MSG_EVT)
  {//切换组播组号
    Group_DstAddr.addrMode = (afAddrMode_t)afAddrGroup;
    Group_DstAddr.endPoint = SAMPLEAPP_ENDPOINT;    
    if(GrounpSwitch == 0)
...
uint8 temp,humi;
uint8 CoordinatorReciveIndicate=0;
uint8 SendToWiFiBuffer[30]={0x30,0x30,0x30,0x30,0x30,0x30,0x30,0x30};
void SampleApp_MessageMSGCB( afIncomingMSGPacket_t *pkt )
{
  ...
  CoordinatorReciveIndicate = 1-CoordinatorReciveIndicate;
  if(CoordinatorReciveIndicate == 0)
  {
    Bottom_Red(); //底座灯亮起蓝色
  }
  else
  {
    Bottom_BlackOut();//底座灯熄灭
  }    
  printf("----->\r\n");
