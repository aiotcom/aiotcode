...
//message event
#define SEND_PERIODIC_MSG_EVT         0x01 
...
void SampleApp_Init( uint8 task_id )
{
  ...
  Point_To_Point_DstAddr.addr.shortAddr = 0x0000;           //协调器的网络短地址为0x0000
...
uint16 SampleApp_ProcessEvent( uint8 task_id, uint16 events )
{
      ...
      {        
        ...
        case ZDO_STATE_CHANGE:         
             ...
             || (SampleApp_NwkState == DEV_END_DEVICE) )
          {
            ...
            //启动定时器，周期性发送数据
            osal_start_timerEx( SampleApp_TaskID, SEND_PERIODIC_MSG_EVT,1000);
          }
          break;
  ...
  if(events & SEND_PERIODIC_MSG_EVT)
  {//时间超时到达，开始周期性发送数据给协调器。
    SendIndicate = 1 - SendIndicate;
    SendIndicate?Bottom_Blue():Bottom_BlackOut();
    
    SendToCoordinator();//向协调器发送数据
    printf("send to Coordinator\r\n");
    osal_start_timerEx( SampleApp_TaskID, SEND_PERIODIC_MSG_EVT,1000);
  }  
  return 0;
}
...
void SendToCoordinator(void)
{
  if ( AF_DataRequest( &Point_To_Point_DstAddr,
                       &EndDevice_epDesc,
                       SEND_TO_COORDINATOR_CLUSTERID, //数据ID号
                       14,                            //数据的发送长度
                       (uint8 *)"Hello world!\r\n",   //要发送的数据内容
                       &SampleApp_TransID,
                       AF_DISCV_ROUTE,
