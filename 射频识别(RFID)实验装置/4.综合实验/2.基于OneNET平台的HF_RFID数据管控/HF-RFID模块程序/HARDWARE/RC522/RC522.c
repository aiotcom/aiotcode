...
//										for(i=0;i<4;i++)
//										{
//											Data_Send[i] = ucArray_ID[i];
//										}
//										for(i=0;i<16;i++)
//										{
//											Data_Send[i+4] = Block_Date[i];
//										}
										...
										macRC522_LED_ON();
										printf("ID:");
										HAL_UART_Transmit(&UART2_Handler,SendData,8,1000);//485发送数据
										printf("\r\n");

										printf("Block5_Date:");
										HAL_UART_Transmit(&UART2_Handler,SendBuf,16,1000);//485发送数据
										printf("\r\n");
										//Rs485_Send(Addr_RFID,Addr_TFT,RFID_All,20,Data_Send);	//发送数据到显示器显示
									}
							}
					}
			}
...
{
	uint8_t SeaCaed_Flag = 1;
	if( MI_OK != PcdRequest ( PICC_REQALL, ucArray_ID ))
	{
		ucStatusReturn = PcdRequest ( PICC_REQALL, ucArray_ID );//寻卡
	}
...
uint8_t Verification_Key(unsigned char ucAuth_mode, unsigned char ucAddr, unsigned char * pKey)
{
	...
	if (PcdAuthState(ucAuth_mode,ucAddr,pKey,ucArray_ID) == MI_OK)//申请对卡的授权  
	{
		SeaCaed_Flag = 0;
	}
	else
...
{     
	...
	GPIO_Initure.Pin=RC522_Reset_PIN|RC522_CS_PIN|RC522_SCK_PIN|RC522_MOSI_PIN; 	//PA0,PA4,PA5,PA7
	GPIO_Initure.Mode=GPIO_MODE_OUTPUT_PP;  	//推挽输出
	GPIO_Initure.Speed=GPIO_SPEED_HIGH;    	 	//高速
	HAL_GPIO_Init(GPIOA,&GPIO_Initure);
	
	GPIO_Initure.Pin=RC522_MISO_PIN; 	        //PA6
	GPIO_Initure.Mode=GPIO_MODE_INPUT;  	    //输入模式
	GPIO_Initure.Speed=GPIO_SPEED_HIGH;    	 	//高速
	HAL_GPIO_Init(RC522_MISO_PORT,&GPIO_Initure);      

	GPIO_Initure.Pin=RC522_LED_PIN; 					//PB14
	GPIO_Initure.Mode=GPIO_MODE_OUTPUT_PP;  	//推挽输出
	GPIO_Initure.Speed=GPIO_SPEED_HIGH;    	 	//高速
	HAL_GPIO_Init(RC522_LED_PORT,&GPIO_Initure);
	macRC522_LED_OFF();
}
