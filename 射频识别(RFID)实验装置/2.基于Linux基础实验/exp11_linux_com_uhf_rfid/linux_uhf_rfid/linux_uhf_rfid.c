...
{   
	...
	msleep(200);//延时10ms			
	if(command_type){
		tmp = (command_type == LED_ON)?1:0;
		Rs485_Send(fd,Addr_Linux,Addr_UHF_RFID,UHF_DATA,1,&tmp);
		command_type = 0;
	}
}
//==========================================================
...
int main(void)
{
	...
	memset((void*)pre_id,0,32);	
  	while(1){ 
		printf("please input cmd \"led_on\" or \"led_off\" \r\n");
		scanf("%s",cmd);
		if(strstr((const char*)cmd,"led_on")){
			command_type = LED_ON;
		}
		else if(strstr((const char*)cmd,"led_off")){
