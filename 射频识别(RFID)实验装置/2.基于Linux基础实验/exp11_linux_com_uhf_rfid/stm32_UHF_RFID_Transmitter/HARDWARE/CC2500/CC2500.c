...
uint8_t  gCC2500TxBuf[32] = {0x06,0x31,0x32,0x33,0x34,0x35,0x36};
...
uint8_t rxstate;
uint8_t rxdelay=0;
ErrorStatus CC2500_RevPacket(uint8_t *len,uint8_t *data)
{
    uint8_t  pktlen,ret;
    if(!READ_GDO0())
    {
    ...
    if((pktlen&NUM_RXBYTES))
    {
        ...
        *len = pktlen;
        ...
        {
            CC2500_BurstRead(RXFIFO,data,pktlen);
...
uint8_t cnt = 0;
void CC2500_SendPacket(uint8_t len,uint8_t *data)
{
    gCC2500TxBuf[0] = len;
    memcpy((void*)&gCC2500TxBuf[1],(const void*)data,len);
    CC2500_BustWrite(TXFIFO,gCC2500TxBuf,len);
//    gCC2500TxBuf[1] = cnt + 0x30;
//    cnt = (++cnt)%10;
