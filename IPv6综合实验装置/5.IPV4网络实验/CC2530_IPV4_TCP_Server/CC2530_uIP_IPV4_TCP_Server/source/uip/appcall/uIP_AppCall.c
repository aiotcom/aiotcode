...
{ 
    ...
    */
    //printf("udp get data\r\n");
    if((pxUDP_Conn->lport))//udp主动发送数据参考
    {
...
extern uint8_t uIP_TCP_Server_RxBuffer[ ];
extern uint8_t uIP_TCP_Server_RxLen;
...
{//定时轮询，每个conn
    ...
    switch(uip_conn->lport) //本地端口
    {
        case HTONS(1200):
            ...
            if(uip_newdata())
            {
                memcpy((void*)uIP_TCP_Server_RxBuffer,(const void*)uip_appdata,uip_len);
                uIP_TCP_Server_RxLen = uip_len;				
            } 
            if(uip_tcp_client_send_len)
            {
