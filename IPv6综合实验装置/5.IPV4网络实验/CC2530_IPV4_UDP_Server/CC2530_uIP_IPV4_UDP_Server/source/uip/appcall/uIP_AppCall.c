...
**************************************************************************/
extern uint8_t uIP_UDP_ServerSendBuffer[50];
extern uint8_t uIP_UDP_ServerSendLen;
extern uint8_t uIP_UDP_Server_RxBuffer[50];
extern uint8_t uIP_UDP_Server_RxLen;
struct _uip_udp_conn *uIP_UDP_ClientLink[1]={0};
struct tcpip_hdr *ptcpip_hdr;
void myudp_appcall(void) 
{ 
    /*
    uip 会轮询当前UDP连接，uip_udp_conn指向当前轮询到的连接
    */
   if(uIP_UDP_ClientLink[0] != (void*)0)
    {
        if(uip_udp_conn == uIP_UDP_ClientLink[0])
        {
            if(uip_newdata())
            {
//                printf("%02x\r\n",*(uIP_UDP_ClientLink[0]->ripaddr));
                printf("get new data\r\n");
                memcpy((void*)uIP_UDP_Server_RxBuffer,(const void*)uip_appdata,uip_len);
//                printf("%s,len=%d\r\n",uip_udp_client_send_buf,uip_len);
                                
                uIP_UDP_Server_RxLen = uip_len;    
            }
            else if(uIP_UDP_ServerSendLen)
            {
                printf("send udp data\r\n");
                memcpy((void*)uip_appdata,(const void*)&uIP_UDP_ServerSendBuffer[0],uIP_UDP_ServerSendLen); 
                uip_send((char *)uip_appdata, uIP_UDP_ServerSendLen);
                uIP_UDP_ServerSendLen = 0;              
            }
                
        }   
    }  
  
   if(uip_newdata()&&(uip_udp_conn->lport == pxUDP_Conn->lport)&&(!uIP_UDP_ClientLink[0]))//pxUDP_Conn 						
   { 
       printf("builded udp conn,udp get data\r\n");
//       printf("rip=%d,rport=%d,%d\r\n",uip_udp_conn->rport,uip_udp_conn->ripaddr[0],uip_udp_conn->ripaddr[1]);
       uIP_UDP_ClientLink[0] = uip_udp_conn; 
       ptcpip_hdr = ((struct tcpip_hdr *)&uip_buf[UIP_LLH_LEN]);
       uip_udp_conn->rport =  ptcpip_hdr->srcport;
       uip_udp_conn->ripaddr[0] =  ptcpip_hdr->srcipaddr[0];
       uip_udp_conn->ripaddr[1] =  ptcpip_hdr->srcipaddr[1];
//       printf("rprt=%d,rip=%x,%x\r\n",ptcpip_hdr->srcport,ptcpip_hdr->srcipaddr[0],ptcpip_hdr->srcipaddr[1]);
//       printf("rip=%d,rip=%x,%x\r\n",uip_udp_conn->rport,uip_udp_conn->ripaddr[0],uip_udp_conn->ripaddr[1]);
		memcpy((void*)uIP_UDP_Server_RxBuffer,(const void*)uip_appdata,uip_len);
		uIP_UDP_Server_RxLen = uip_len; 
   }
    ...
    switch(uip_conn->lport) //本地端口
    {
        case HTONS(1200):
            ...
            {//如果有客户端接入，走到这里。client_link_uip_conn 保存客户端的连接
                client_link_uip_conn[0] = uip_conn;
            }
            if(uip_newdata())
            {
