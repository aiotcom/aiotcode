...
{
    static uint32_t i = 0; 
    static uint8_t OLED_Fig = 1;
    static float pitch,roll,yaw; //欧拉角
    static uint8_t Pitch[10],Roll[10],Yaw[10];
...
*/
    ...
    {
        sprintf((char *)Pitch,"%0.2f",pitch);
        sprintf((char *)Roll,"%0.2f",roll);
        sprintf((char *)Yaw,"%0.2f",yaw);
        Pitch[8] = '\0';Roll[8] = '\0';Yaw[8] = '\0';

        delay_ms(2);

        OLED_P8x16Str(10,2,"Pitch:");	//Pitch
        OLED_P8x16Str(65,2,Pitch);
        
        OLED_P8x16Str(10,4,"Roll :");	//Roll
        OLED_P8x16Str(65,4,Roll);
        
        OLED_P8x16Str(10,6,"Y a w:");	//Yaw 
        OLED_P8x16Str(65,6,Yaw);

        for(i=0;i<10;i++)
        {
            Pitch[i] = Roll[i] = Yaw[i] = 0;//清空数组
        }
    }
}
